...
# List offer a lot of flexibility.
my_list = [[1, 2, 3], 'a', ['x', 'y'], [3/5]]
...
# Note that the iterator remains in memory
# with the last value.
print(whatever)
...
# A better way:
i = 5
while i > 0 and not i == 3:
  i -= 1
  print('i = %d' % i)
  print("Inside the loop")

# Not exactly the same.
# This version executes all commands in the block.
...
def FeetToMetres3(feet):
    centi = 2.54
    inches = 12
    metres = feet*inches*cent/100
    return metres
...
# Try to avoid using variables that could be mispelled.
print(meters)
...
def OddOrEven_2(number):
    if type(number) == int:
        ...
        print('Error: Input number is not an int')
